...
/**
 * Find Locally Resolver.
 * 
 * @author debbabi
 *
 */
public class FindLocallyResolver implements IConstraintResolver {

	/**
	 * {@inheritDoc}
	 */
	public String getConstraintName() {
		return FindLocally.NAME;
	}
	...
	public RValue find(List<RVariable> params, int which, Constraint c) {		
		//System.out.println("\n\n[FindLocally] find ("+which+") ...\n\n");
		if (which == 0) {
			RVariable v1 = params.get(0);
			if (v1 != null) {
				CubeAgent agent = v1.getResolutionGraph().getCubeAgent();		
				List<CInstance> instances = agent.getRuntimeModel().getCInstances(v1.getType());
				if (instances != null && instances.size() > 0) {
					for (CInstance i: instances) {			
						if (i.getState()==CInstance.VALID) {
							if (!v1.isInHistory(i.getId())) {
								System.out.println("\n[INFO] FindLocallyResolver : finding " + i.getId());
								return new RValue(i, this);	
							}					
						}				
					}
				}
